...
<html lang="en">
  ...
  <body>
    ...
    <?php
    $sql = "SELECT * FROM `blog_posts` ORDER BY `blog_posts`.`blog_id` DESC";
    ...
    while($row = mysqli_fetch_assoc($result)){
      ...
      <div class="blog">
          ...
          <h2 class="title" style="padding: 0 30px 0 0; width: 600px;">' . $title . '</h2>
          <ul class="text-start">
          <h5 class="card-title">' . $author_name . '</h5>
          <h6 class="card-subtitle mb-2 text-muted"></h6>
          </ul>
          </div>
          <p class="descripton">' . substr($content, 0, 455) . '...</p>
          <a id="btn" class="btn btn-success" href="blog_post.php?blog_id=' . $blog_id . '">Read More</a>
      </div>
  </div>';
    }
    ?>
